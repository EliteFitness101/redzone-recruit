...
export const Referral = () => {
  const { user } = useAuth();
  const [code, setCode] = useState("SIGN-UP-FIRST");
  ...
  const link = user ? `${SITE.domain}/register?ref=${code}` : "";
          ...
          <div className="relative grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <h2 className="font-display text-4xl md:text-5xl font-bold leading-tight">
                Recruit a brother. <span className="text-gradient-gold">Earn 10% + ₦2,500</span>.
              </h2>
              <p className="mt-4 text-muted-foreground max-w-md">
                Every paid signup through your code earns instant rewards. Top recruiters get free VIP upgrades and dedicated deployment slots.
              </p>
              ...
              {!user && (
                <Button variant="gold" size="lg" className="mt-6" asChild>
                  <Link to="/register" onClick={() => track("cta_click", { source: "referral_signup" })}>
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <div className="mt-2 flex items-center justify-between gap-3">
                <span className="font-tactical text-3xl md:text-4xl text-gradient-gold tracking-wider">{code}</span>
                <Button variant="tactical" size="sm"
                  disabled={!user}
                  onClick={() => {
                    navigator.clipboard.writeText(link || code);
                    toast.success("Copied");
                    track("cta_click", { source: "referral_copy" });
                  }}>
                  <Copy className="h-4 w-4" /> Copy
                </Button>
              </div>
              {user && (
                <div className="mt-3 text-xs text-muted-foreground break-all">{link}</div>
              )}
              <div className="mt-6 grid grid-cols-3 gap-3 text-center">
                {[{ v: "10%", l: "commission" }, { v: "₦2.5k", l: "bonus" }, { v: "VIP", l: "milestone" }].map((s) => (
              ...
              {user && (
                <Button variant="hero" className="w-full mt-4" size="sm" asChild>
                  <Link to="/referrals">View earnings</Link>
                </Button>
              )}
            </div>
          </div>
