      ...
      <div className="container">
        ...
        {loading ? (
          <div className="grid lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => <div key={i} className="glass rounded-3xl p-8 h-[430px] animate-pulse" />)}
          </div>
        ) : offers.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {offers.map((t) => (
              <div key={t.id} className={cn("relative rounded-3xl p-8 hover-lift", t.featured ? "glass-strong border-gold/40 shadow-gold" : "glass")}>
                ...
                <div className={cn("h-12 w-12 rounded-xl flex items-center justify-center mb-5", t.featured ? "bg-gradient-gold shadow-gold" : "bg-gradient-red shadow-red")}>
                  {t.id === "basic" ? <Shield className="h-6 w-6 text-primary-foreground" /> : t.id === "elite" ? <Zap className="h-6 w-6 text-accent-foreground" /> : <Crown className="h-6 w-6 text-primary-foreground" />}
                </div>
                ...
                <Button variant={t.variant} size="lg" className="w-full mt-8" asChild>
                  <Link to={`/checkout?tier=${t.id}`} onClick={() => track("cta_click", { source: `pricing_${t.id}`, sku: t.sku, value: t.price, currency: "NGN" })}>{t.cta}</Link>
                </Button>
              </div>
            ))}
          </div>
