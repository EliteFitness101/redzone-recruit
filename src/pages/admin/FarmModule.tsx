...
const config: Record<string,{title:string;eyebrow:string;table:string;fields:string[]}> = {
  ...
  assurance:{title:"Recruitment Assurance & Risk Register",eyebrow:"Recruitment Risk Governance",table:"farm_recruitment_assurance",fields:["assurance_type","trigger_scope","coverage_description","coverage_limit","currency","guarantee_period_days","replacement_period_days","exclusions","claim_procedure","provider_name","provider_license_reference","policy_or_bond_reference","status","approval_reference","evidence_url","notes"]}
};
...
export default function FarmModule(){
  ...
  const save=async(e:FormEvent)=>{
    ...
    if(["required_quantity","workers_present","workers_absent","tasks_planned","tasks_completed","tasks_delayed","fee_base","proposed_amount","coverage_limit","guarantee_period_days","replacement_period_days"].some(f=>f in clean)) {
